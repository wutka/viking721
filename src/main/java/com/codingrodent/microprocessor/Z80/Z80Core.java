...
public class Z80Core implements ICPUData {
    ...
    public void executeOneInstruction() throws ProcessorException {
        ...
        if (NMI_FF) {
            ...
            int vec_lower = io.getINTACK() & 0xff;
            int vec = vec_lower | (reg_I << 8);
            ram.writeWord(reg_SP, reg_PC);
            reg_PC = vec;
        }
        halt = false;
    ...
    private void OUTI() {
        int outval = ram.readByte(getHL());
        io.IOWrite(reg_C, outval & 0xff);
        reg_R++;
        reg_B = (reg_B - 1) & lsb;
        setHL(ALU16BitInc(getHL()));
        int t = (outval&0xff) + (reg_L & 0xff);
        reg_F = reg_B & 0x28;
        setZ(reg_B == 0);
        if ((outval & reg_F_ALT) != 0) {
            setN();
        }
        if (t > 0xff) {
            setC();
        ...
        setPV(PARITY_TABLE[((t & 0x07) ^ reg_B) & 0xff]);
    }
